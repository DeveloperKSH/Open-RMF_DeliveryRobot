...
class FleetManager(smach.State):
    ...
    def execute(self, userdata):
        ...
        self.sub1_ = self.node.create_subscription(
            ...
            qos_profile=transient_qos)
        '''
        ros2 topic pub -1 --keep-alive 5 /door std_msgs/msg/String "data: 'true'" --qos-durability transient_local
        '''
    ...
    def on_message(self, ws, message):
        ...
        try:
            ...
            if package.get('type') == 'connected':
                ...
                # info(f'response: {response}')
            elif "response" in package and package["response"].get("type") == "door":
    ...
    def door_callback(self, msg):
        value = msg.data.lower()
        if value not in ["true", "false"]:
            error(f"Invalid value received: {value}")
            return
        ...
        temp = [
            {
                'request': {
                    ...
                    "robot_name": "tinybot1",
                    "type": "door",
                    "value": value,  # `/door` 토픽에서 받은 값을 반영
                    "patrol": "patrol_A1"
                }
            }
        ]
